...
def get_variant_counts(
    ...
    var_fps = get_filepaths(
        analysis_filepath,
        data_fmt="tsv",
        data_type="variants/illumina",
        sample_ids=search_ids,
        generalised=generalised,
        return_type="list",
    )
    var_df = pd.concat(
        [
            pd.read_csv(fp, sep="\t").assign(sample_id=fp.split("/")[-1].split("_")[0])
            for fp in var_fps
        ]
    )
    var_df = var_df.loc[
        (var_df["ALT_FREQ"] < max_freq) & (var_df["ALT_FREQ"] > min_freq)
    ]
    var_df["variant"] = var_df["POS"].astype(str) + ":" + var_df["ALT"].astype(str)
    return (
        var_df.groupby("sample_id")
        .agg(num_nt_mutations=("variant", "nunique"))
        .sort_index()
    )
...
def separate_samples(
    ...
    results["_private_sheet"] = pd.concat(
        [
            sample_sheet.loc[sample_sheet["Sample_ID"].str.contains(idx)]
    ...
    for primer_idx in primer_ids:
        results[f"{primer_idx}_primers"] = sample_sheet.loc[
            sample_sheet["I7_Index_ID"].str.contains(primer_idx)
        ]
        for sample_idx in results[f"{primer_idx}_primers"]["Sample_ID"].unique():
            samples_to_exclude.append(sample_idx)
    results["OG_primers"] = sample_sheet.loc[
        ~sample_sheet["Sample_ID"].isin(samples_to_exclude)
    ]
    return results
...
def copy_files(filenames: list, destination_dir):
    ...
    try:
        ...
        raise ValueError(
            "Copy operation failed. Check if all filenames and directories exist"
        )


def generate_release_report(out_dir, report_name="release_report.tar"):
    report_fp = out_dir / report_name
    s1 = "find {out_dir}/ -name '*.csv' -type f -exec tar rfP {report_fp} {{}} \\;".format(
        out_dir=out_dir, report_fp=report_fp
    )
    print(s1)
    run_command(s1)
...
def separate_alignments(
    msa_data, sus_ids, out_dir, filename, patient_zero="NC_045512.2"
):
    good_seqs = []
    poor_seqs = []
    for rec in msa_data:
...
def integrate_gisaid_meta(old_meta_fp, xtra_fp, msa_fp, rename_cols, drop_cols):
    ...
    meta = meta.loc[
        ~(
            (meta["country"].str.contains("USA"))
            & (meta["pangolin_lineage"] == "B.1.1.7")
        )
    ]
...
def fetch_seqs(
    seqs_filepath, out_fp, sample_idxs: list, is_aligned=False, is_gzip=False
):
    if is_aligned:
        if is_gzip:
        ...
        my_cns = Align.MultipleSeqAlignment(
            [rec for rec in cns if rec.id in sample_idxs]
        )
    ...
    if sample_ids:
        for s_id in sample_ids:
            ...
            if data_type and tech:
                f = glob.glob(
                    f"{analysis_path}{generalised_flag}/*{data_type}*/*{tech}*/*{s_id}*.{data_fmt}"
                )
            elif data_type:
                f = glob.glob(
                    f"{analysis_path}{generalised_flag}/*{data_type}*/*{s_id}*.{data_fmt}"
                )
            else:
        ...
        if data_type and tech:
            if generalised:
                fs = glob.glob(
                    f"{analysis_path}/**/*{data_type}*/*{tech}*/*.{data_fmt}"
                )
            else:
...
def get_variant_filepaths(
    sample_ids: list, analysis_path: str = "/home/gk/analysis"
) -> dict:
...
def get_variant_data(variant_filepaths: dict):
    ...
    df = pd.concat(
        (
            pd.read_csv(f, sep="\t").assign(sample=s_id, location=find_loc(f))
            for s_id, f in variant_filepaths.items()
        )
    )
    return df
...
def run_minimap2(in_filepath, out_filepath, ref_path, num_cpus=25):
    map_cmd = (
        f"minimap2 -a -x asm5 -t {num_cpus} {ref_path} {in_filepath} -o {out_filepath}"
    )
    run_command(map_cmd)
    return out_filepath
...
def gofasta_align(fasta_filepaths, indiv_out_filepath, out_filepath):
    ...
    for file in files:
        ...
        pwa_cmd = f"minimap2 -a -x asm20 --score-N=0 /home/gk/code/hCoV19/db/NC045512.fasta {os.path.abspath(file)} | gofasta sam topa > {out_file}"
        run_command(pwa_cmd)
...
def align_fasta_viralMSA(
    fasta_filepath,
    out_filepath,
    ref_fp: str,
    num_cpus=8,
    email="notmyemail@noway.com",
...
def add_gene_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        df.loc[df["gene"] == "nan", "gene"] = df.loc[df["gene"] == "nan", "POS"].apply(
            map_gene_to_pos
        )
    except:
